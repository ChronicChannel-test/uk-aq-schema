...
Queue for throttled LA lookups.
...
## Notes
- Rows are queued when a station has geometry but `la_code` or `la_version` is null.
- The worker processes a small batch each run via `uk_aq_process_station_la_queue`.
