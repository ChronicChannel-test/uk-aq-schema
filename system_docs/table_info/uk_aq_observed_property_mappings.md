...
the policy to `phenomena.observed_property_id`. Normal ingest calls consume
authoritative policy; controlled registration/migration calls must explicitly
enable mapping upsert mode. Unknown source labels remain unmapped and are
returned with a diagnostic warning.

Phase 5 preserves verified mappings for Breathe London Communities, OpenAQ,
Sensor.Community, and UK-AIR SOS as connector policy with
`confidence = legacy_backfill`. AQ-domain phenomena are classified as raw
observed properties, meteorological-domain phenomena as meteorological, and
only PM2.5, PM10, and NO2 are AQI eligible. Legacy phenomena without a stable
`source_label` are excluded.
