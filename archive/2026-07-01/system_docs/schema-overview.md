...
- Service preflight view:
  - `uk_aq_public.stations_fk_check` exposes only mirrored station `id` values
    to `service_role`, allowing FK validation through the exposed public schema
    without exposing `uk_aq_core` over PostgREST.
