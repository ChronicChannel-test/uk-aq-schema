-- Move DB size metrics storage from uk_aq_raw to uk_aq_ops in ingest DB.
...
drop view if exists uk_aq_public.uk_aq_db_size_metrics_hourly;

do $$
begin
  if to_regclass('uk_aq_raw.db_size_metrics_hourly') is not null
     and to_regclass('uk_aq_ops.db_size_metrics_hourly') is null then
    execute 'alter table uk_aq_raw.db_size_metrics_hourly set schema uk_aq_ops';
  end if;
end
$$;
...
do $$
begin
  if to_regclass('uk_aq_raw.db_size_metrics_hourly') is not null then
    ...
    execute 'drop table uk_aq_raw.db_size_metrics_hourly';
  end if;
end
$$;
