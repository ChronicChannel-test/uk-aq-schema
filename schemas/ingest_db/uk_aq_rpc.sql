...
returns setof uk_aq_core.observations
language plpgsql
security invoker
set search_path = uk_aq_core, public, pg_catalog
as $$
...
create or replace function uk_aq_public.uk_aq_rpc_r2_history_days_by_domain(
  p_from_day_utc date,
  p_to_day_utc date
)
returns table (
...
begin
  ...
  select distinct
    g.day_utc::date as day_utc,
    (regexp_match(g.history_manifest_key, '^history/v1/([^/]+)/'))[1] as domain_name
  from uk_aq_ops.prune_day_gates g
  where g.day_utc >= p_from_day_utc
    and g.day_utc <= p_to_day_utc
    and g.history_done is true
...
revoke all on function uk_aq_public.uk_aq_rpc_r2_history_days_by_domain(date, date) from public;
grant execute on function uk_aq_public.uk_aq_rpc_r2_history_days_by_domain(date, date) to service_role;

-- Phase 1 additive: timeseries-first AQI source/helper RPCs.
...
drop function if exists uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_upsert(
  timestamptz,
  timestamptz,
  integer[],
  timestamptz
...
create or replace function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_upsert(
...
begin
  ...
  with source_rows as (
    select
      ...
      s.hourly_mean_ugm3,
      s.sample_count
    from uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_source(
      v_source_start,
      v_source_end,
      p_timeseries_ids
    ) s
  ...
  hourly as (
    ...
    left join source_rows sr
      on sr.timeseries_id = g.timeseries_id
     and sr.timestamp_hour_utc = g.timestamp_hour_utc
  ),
  with_rolling as (
    select
      h.timeseries_id,
      ...
      avg(
        case
          when h.pollutant_code = 'pm25' then h.hourly_mean_ugm3
          else null
        end
      ) over w as pm25_rolling24h_mean_raw,
      count(
        case
          when h.pollutant_code = 'pm25' then h.hourly_mean_ugm3
          else null
        end
      ) over w as pm25_rolling24h_valid_hours_raw,
      avg(
        case
          when h.pollutant_code = 'pm10' then h.hourly_mean_ugm3
          else null
        end
      ) over w as pm10_rolling24h_mean_raw,
      count(
        case
          when h.pollutant_code = 'pm10' then h.hourly_mean_ugm3
          else null
        end
      ) over w as pm10_rolling24h_valid_hours_raw
    from hourly h
    window w as (
      partition by h.timeseries_id
  ...
  target_hours as (
    select
      ...
      case
        when wr.pollutant_code = 'no2' then wr.hourly_mean_ugm3
        else null
      end as no2_hourly_mean_ugm3,
      case
        when wr.pollutant_code = 'pm25' then wr.hourly_mean_ugm3
        else null
      end as pm25_hourly_mean_ugm3,
      case
        when wr.pollutant_code = 'pm10' then wr.hourly_mean_ugm3
        else null
      end as pm10_hourly_mean_ugm3,
      case
        when wr.pollutant_code = 'pm25' and wr.pm25_rolling24h_valid_hours_raw >= 18
          then wr.pm25_rolling24h_mean_raw
        else null
      end as pm25_rolling24h_mean_ugm3,
      case
        when wr.pollutant_code = 'pm10' and wr.pm10_rolling24h_valid_hours_raw >= 18
          then wr.pm10_rolling24h_mean_raw
        else null
  ...
  upserted as (
    insert into uk_aq_aqilevels.timeseries_aqi_hourly_helper (
      timeseries_id,
      station_id,
      connector_id,
      pollutant_code,
...
drop function if exists uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_window(
  timestamptz,
  timestamptz,
  integer[]
);

create or replace function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_window(
  p_hour_end_start_exclusive timestamptz,
  p_hour_end_end_inclusive timestamptz,
  p_timeseries_ids integer[] default null
)
...
revoke all on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_source(
  timestamptz,
  timestamptz,
  integer[]
) from public;
...
revoke all on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_upsert(
  timestamptz,
  timestamptz,
  integer[],
  timestamptz
...
grant execute on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_upsert(
  timestamptz,
  timestamptz,
  integer[],
  timestamptz
...
revoke all on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_window(
  timestamptz,
  timestamptz,
  integer[]
) from public;
...
grant execute on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_window(
  timestamptz,
  timestamptz,
  integer[]
) to service_role;

revoke all on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_cleanup(integer) from public;

revoke all on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_cleanup(integer) from anon, authenticated;

grant execute on function uk_aq_public.uk_aq_rpc_timeseries_aqi_hourly_helper_cleanup(integer) to service_role;

-- Phase 2 hard cut: remove legacy station-focused AQI RPC surface.
