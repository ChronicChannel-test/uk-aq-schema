...
begin
  for t in
    select unnest(ARRAY['uk_air_sos_site_register','laqn_site_register','uk_air_sos_station_refs','breathelondon_station_checkpoints','erg_laqn_station_checkpoints','uk_air_sos_timeseries_checkpoints','openaq_station_checkpoints','openaq_timeseries_checkpoints','openaq_request_budget_minute','error_logs']::text[])
  loop
...
begin
  ...
  loop
    ...
    if not exists (
      ...
      execute format(
        'create policy %I on uk_aq_pop.%I for select using ((select auth.role()) in (''authenticated'',''service_role''));',
        t || '_select_authenticated', t
      );
    end if;
    if not exists (
      ...
      execute format(
        'create policy %I on uk_aq_pop.%I for all using ((select auth.role()) = ''service_role'') with check ((select auth.role()) = ''service_role'');',
        t || '_write_service_role', t
      );
    end if;
  end loop;
