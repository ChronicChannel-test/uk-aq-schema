...
-- 1) Capture/drop dependent views and materialized views.
create temporary table _int8_dependent_views (
...
insert into _int8_dependent_views (
  view_oid,
  schema_name,
  view_name,
  relkind,
...
create temporary table _int8_observations_partitions (
...
begin
  if exists (
    ...
    delete from _int8_observations_partitions;

    insert into _int8_observations_partitions (
      schema_name,
      old_partition_name,
      new_partition_name,
      bound_expr
    ...
    select
      ...
      format('%s__i8m_%s', left(child.relname, 40), child.oid::text) as new_partition_name,
    ...
    execute $q$
      create table uk_aq_core.observations__int8_mig (
    ...
    for v_part in
      ...
      from _int8_observations_partitions
      order by ord
    loop
      execute format(
        'create table %I.%I partition of uk_aq_core.observations__int8_mig %s',
        v_part.schema_name,
    ...
    execute $q$
      insert into uk_aq_core.observations__int8_mig (
        connector_id,
        timeseries_id,
        observed_at,
        value,
    ...
    execute 'alter table uk_aq_core.observations__int8_mig rename to observations';
...
-- 3) Alter target columns back to bigint.
do $$
declare
  v_col record;
begin
...
-- 5) Recreate dropped views/materialized views.
do $$
declare
  v_row record;
  v_opt text;
