-- Ingest DB migration: rewrite RLS policy predicates to use initplan-safe auth role checks.
-- Run in ingestdb.
...
begin
  for p in
    select
      ...
      with_check
    from pg_policies
    where schemaname in ('public', 'uk_aq_core', 'uk_aq_raw', 'uk_aq_pop', 'uk_aq_aqilevels', 'uk_aq_ops')
      and (
